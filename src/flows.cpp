...
flow_t generate_flow(uint32_t index, uint16_t pkt_size) {
    ...
    ip_hdr->total_length = rte_cpu_to_be_16(pkt_size_no_crc - sizeof(struct rte_ether_hdr));
    ip_hdr->packet_id = 0;
    ...
    udp_hdr->dgram_len = rte_cpu_to_be_16(
        pkt_size_no_crc - (sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr)));
